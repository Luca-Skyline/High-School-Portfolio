...
NumButton[] leftNums = new NumButton[10];
NumButton[] rightNums = new NumButton[10];
Button[] operators = new Button[7];
Button[] leftAdjusts = new Button[5];
Button[] rightAdjusts = new Button[5];
...
void draw() {
  ...
  for (int i = 0; i < rightAdjusts.length; i++) {
  ...
  for (int i = 0; i < leftAdjusts.length; i++) {
...
void keyPressed() {
  ...
  if (keyCode == 107 || keyCode == 61) {
    ...
    left = false;
  } else if (left) { //if should work on left
...
void handleLeftEvent(char val, boolean num) {
  if (num && dL.length() < 15) {
...
void handleRightEvent(char val, boolean num) {
  if (num && dR.length() < 15) {
